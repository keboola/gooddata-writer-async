...
class RestApi
{
    ...
    public function __construct($appName, Logger $logger)
    {
        $this->logger = $logger;
    ...
    public function cloneProject($pidSource, $pidTarget, $includeData, $includeUsers)
    {
        ...
        $result = $this->jsonRequest(sprintf('/gdc/md/%s/maintenance/import', $pidTarget), 'POST', [
            'importProject' => [
    ...
    public function validateProject($pid, $validate = ['pdm'])
    {
        $result = $this->jsonRequest(sprintf('/gdc/md/%s/validate', $pid), 'POST', ['validateProject' => $validate]);
    ...
    public function loadData($pid, $dirName)
    {
        ...
        $result = $this->jsonRequest($uri, 'POST', ['pullIntegration' => $dirName]);
    ...
    {
        $dataSetModel = Model::getLDM($definition, $noDateFacts);
        ...
        if (!$dataSetFound) {
            $model['projectModel']['datasets'][] = ['dataset' => $dataSetModel];
        }
    ...
    public function generateUpdateProjectMaql($pid, $model)
    {
        try {
            ...
            $result = $this->jsonRequest($uri, 'POST', ['diffRequest' => ['targetModel' => $model]]);
    ...
    {
        $this->request('/gdc/xtab2/executor3', 'POST', [
            'report_req' => [
                'report' => $uri
            ]
        ]);
    ...
    {
        return $this->request(sprintf('/gdc/app/projects/%s/execute/raw/', $pid), 'POST', [
            "report_req" => [
    ...
    public function executeMaql($pid, $maql)
    {
        ...
        do {
            ...
            $try++;
        } while (in_array($taskResponse['wTaskStatus']['status'], ['PREPARED', 'RUNNING']));
    ...
    {
        $gdc = $this->get('/gdc');
        if (isset($gdc['about']['links'])) {
            foreach ($gdc['about']['links'] as $link) {
    ...
    {
        $result = $this->post(sprintf('/gdc/md/%s/etl/mode', $pid), [
            'etlMode' => [
                'mode' => 'SLI',
                'lookup' => 'recreate'
            ]
     ...
     */
    protected function jsonRequest($uri, $method = 'GET', $params = [], $headers = [])
    {
    ...
    public function getToFile($uri, $filename)
    {
        ...
        $request = $this->client->get($this->apiUrl . $uri, [
            'accept' => 'text/csv',
     ...
     */
    private function request($uri, $method = 'GET', $params = [], $headers = [], $refreshToken = true)
    {
    ...
    public function login($username, $password)
    {
        ...
        try {
            $response = $this->request('/gdc/account/login', 'POST', [
                'postUserLogin' => [
                    'login' => $username,
                    'password' => $password,
                    'remember' => 0
