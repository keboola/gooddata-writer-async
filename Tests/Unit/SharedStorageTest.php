...
class SharedStorageTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testJobs()
    {
        ...
        // createJob()
        $this->sharedStorage->createJob($jobId, $projectId, $writerId, [
            'command' => $command,
            'token' => uniqid(),
            'tokenId' => rand(1, 128),
            'tokenDesc' => uniqid(),
        ...
        // saveJob()
        $command = uniqid();
        $this->sharedStorage->saveJob($jobId, ['command' => $command]);
        ...
        // batchToApiResponse()
        $response = SharedStorage::batchToApiResponse($jobId, [$job]);
